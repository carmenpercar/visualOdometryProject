...
class DataHandler:
    def __init__(self):
        ...
        # os.path.dirname(path): return the directory name of the path

        # os.path.realpath(__file__):Return the canonical path of the specified filename, eliminating any symbolic
        # links encountered in the path

        # Data location
        root_dir_path = os.path.dirname(os.path.realpath(__file__))
        self.image_dir = os.path.join(root_dir_path, 'data_frames')
        self.depth_dir = os.path.join(root_dir_path, 'frames_depth')
    ...
    def read_image(self):
        for i in range(1, self.num_frames + 1):
            ...
            print("Data loading: {0}%".format(int((i+self.num_frames)/(2*self.num_frames + 1)*100)), end="\r")
